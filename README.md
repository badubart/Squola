...
![Testy](https://img.shields.io/badge/testy-164%20passing-brightgreen)
...
„pobierz wszystkie zajęcia". Każda klasa posiada **testy jednostkowe**, a kluczowe
procesy biznesowe — **testy akceptacyjne**.
...
├─ test/
│  └─ org/scuola/
│     ├─ users/ lessons/ attendance/ notifications/ repository/   # testy jednostkowe (pakiety lustrzane)
│     ├─ fixtures/                   # TestFixtures — wspólne dane testowe (Object Mother)
│     └─ acceptance/                 # testy akceptacyjne (scenariusze / user stories)
├─ lib/
...
- 40 klas testowych + fixture (`TestFixtures`) = 164 testy (JUnit 5).
...
| --- | --- | --- |
| **Jednostkowe** | `org.scuola.*` (lustrzane do `src`) | Każda klasa testowana w izolacji. |
| **Akceptacyjne** | `org.scuola.acceptance` | Pełne procesy biznesowe (user stories) w stylu *Given / When / Then*. |
| **Fixture** | `org.scuola.fixtures` | `TestFixtures` — z gotowymi obiektami domenowymi. |
